    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Convert waypoints coordinates to vehicle coordinates
          ...
          for(int i = 0; i < ptsx.size(); ++i) {
            ...
            y_waypoints[i] = dy * cos(-psi) + dx * sin(-psi);
          }

          // Fit a third degree polynomial to the waypoints
          auto coeffs = polyfit(Eigen::Map<Eigen::VectorXd>(x_waypoints.data(),x_waypoints.size()),
                                Eigen::Map<Eigen::VectorXd>(y_waypoints.data(),y_waypoints.size()), 3);
          ...
          // Otherwise the values would be in between [-deg2rad(25), deg2rad(25] instead of [-1, 1].
